...
/* ================== 核心修复区域 ================== */
/* 增加一个转接层（Wrapper），用于解决 GKI 5.15 内核要求 unsigned int，
   但原有 rwmem_bp_step_handler 是 unsigned long 的类型冲突问题 */
static int rwmem_bp_step_handler_wrapper(struct pt_regs *regs, unsigned int esr)
{
	// 强转回 unsigned long 传给原函数，完美绕过类型检查且不影响执行
	return rwmem_bp_step_handler(regs, (unsigned long)esr);
}

static struct step_hook rwmem_bp_step_hook = {
	.fn = rwmem_bp_step_handler_wrapper,
};
/* ================================================= */
